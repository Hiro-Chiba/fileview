...
mod image_preview_tests {
    use super::*;
    use fileview::render::{create_image_picker, ImagePreview, Picker};
    use std::fs;

    // -------------------------------------------------------------------------
    // Image File Detection - Extended Tests
    // -------------------------------------------------------------------------

    #[test]
    fn test_image_file_detection_all_supported_formats() {
        // All formats supported by ratatui-image
        let supported = [
            "image.png",
            "image.jpg",
            "image.jpeg",
    ...
    #[test]
    fn test_image_file_detection_case_variations() {
        // Upper case
        assert!(is_image_file(&PathBuf::from("IMAGE.PNG")));
        assert!(is_image_file(&PathBuf::from("IMAGE.JPG")));
        assert!(is_image_file(&PathBuf::from("IMAGE.JPEG")));
    ...
    #[test]
    fn test_image_file_detection_with_path() {
        // Full path
        assert!(is_image_file(&PathBuf::from("/path/to/image.png")));
        assert!(is_image_file(&PathBuf::from("./relative/image.jpg")));
        assert!(is_image_file(&PathBuf::from("../parent/image.gif")));
    ...
    #[test]
    fn test_image_file_detection_multiple_dots() {
    ...
    #[test]
    fn test_image_file_detection_not_image() {
        // Text files
        assert!(!is_image_file(&PathBuf::from("file.txt")));
    ...
    #[test]
    fn test_image_file_detection_hidden_files() {
        // Hidden image files
    ...
    #[test]
    fn test_image_file_detection_similar_extensions() {
        // Extensions that look similar but are not images
        assert!(!is_image_file(&PathBuf::from("file.pn"))); // Not .png
        assert!(!is_image_file(&PathBuf::from("file.jp"))); // Not .jpg
        assert!(!is_image_file(&PathBuf::from("file.pngg"))); // Extra character
        assert!(!is_image_file(&PathBuf::from("file.jpgg"))); // Extra character
    }
    ...
    #[test]
    fn test_create_image_picker_returns_option() {
        // create_image_picker should return Option<Picker>
        // It may return None in CI environments without proper terminal
        let picker = create_image_picker();

        // We can't assert success in CI, but we can verify it doesn't panic
        // and returns a valid Option type
        match picker {
            Some(_) => {
                // Picker was created successfully (terminal supports queries)
            }
            None => {
                // Picker creation failed (expected in CI or unsupported terminals)
            }
        }
    }

    #[test]
    fn test_create_image_picker_is_deterministic() {
        // Multiple calls should give consistent results
        let picker1 = create_image_picker();
        let picker2 = create_image_picker();

        // Both should be the same variant (Some or None)
        assert_eq!(picker1.is_some(), picker2.is_some());
    }
    ...
    /// Create a minimal valid PNG file (1x1 pixel) using image crate
    fn create_test_png(path: &std::path::Path) {
        use image::{ImageBuffer, Rgb};

        // Create a 1x1 red image
        let img: ImageBuffer<Rgb<u8>, Vec<u8>> =
            ImageBuffer::from_fn(1, 1, |_, _| Rgb([255, 0, 0]));
        img.save(path).unwrap();
    }

    #[test]
    fn test_image_open_valid_png() {
        let temp = TempDir::new().unwrap();
        let png_path = temp.path().join("test.png");
        create_test_png(&png_path);

        // Verify the file can be opened by the image crate
        let result = image::open(&png_path);
        assert!(
            result.is_ok(),
    ...
    #[test]
    fn test_image_detection_with_real_file() {
        let temp = TempDir::new().unwrap();
        let png_path = temp.path().join("real_image.png");
        create_test_png(&png_path);

        // File should be detected as image
        assert!(is_image_file(&png_path));

        // File should NOT be detected as text or binary
        assert!(!is_text_file(&png_path));
        assert!(!is_binary_file(&png_path));
    }

    #[test]
    fn test_image_preview_load_with_picker() {
        let temp = TempDir::new().unwrap();
        let png_path = temp.path().join("preview_test.png");
        create_test_png(&png_path);

        // Try to create picker and load image
        if let Some(mut picker) = create_image_picker() {
            let result = ImagePreview::load(&png_path, &mut picker);
            assert!(
                result.is_ok(),
        ...
        // If picker is None, skip this test (CI environment)
    }

    #[test]
    fn test_image_preview_load_nonexistent_file() {
        if let Some(mut picker) = create_image_picker() {
    ...
    #[test]
    fn test_image_preview_load_invalid_image() {
        let temp = TempDir::new().unwrap();
        let fake_path = temp.path().join("not_an_image.png");

        // Create a file that has .png extension but is not a valid PNG
        fs::write(&fake_path, "This is not a PNG file").unwrap();

        if let Some(mut picker) = create_image_picker() {
            let result = ImagePreview::load(&fake_path, &mut picker);
            assert!(result.is_err(), "Expected error for invalid image file");
        }
    }

    #[test]
    fn test_image_preview_load_empty_file() {
        let temp = TempDir::new().unwrap();
        ...
        if let Some(mut picker) = create_image_picker() {
            let result = ImagePreview::load(&empty_path, &mut picker);
    ...
    /// Create a minimal valid JPEG file using image crate
    fn create_test_jpeg(path: &std::path::Path) {
        use image::{ImageBuffer, Rgb};

        // Create a 1x1 blue image and save as JPEG
        let img: ImageBuffer<Rgb<u8>, Vec<u8>> =
    ...
    #[test]
    fn test_image_open_valid_jpeg() {
        let temp = TempDir::new().unwrap();
        let jpeg_path = temp.path().join("test.jpg");
        create_test_jpeg(&jpeg_path);

        let result = image::open(&jpeg_path);
        assert!(
            result.is_ok(),
            "Failed to open test JPEG: {:?}",
            result.err()
        );
    }
    ...
    #[test]
    fn test_image_file_unicode_filename() {
    ...
    #[test]
    fn test_render_module_exports_image_types() {
        // Verify that all expected types are exported from render module
        // This test ensures the public API is stable

        // Type aliases to verify exports
        let _: fn(&std::path::Path, &mut Picker) -> anyhow::Result<ImagePreview> =
            ImagePreview::load;
        let _: fn() -> Option<Picker> = create_image_picker;

        // Verify is_image_file is exported
        let _: fn(&std::path::Path) -> bool = is_image_file;
    }

    // -------------------------------------------------------------------------
    // Integration with Other Preview Types
    // -------------------------------------------------------------------------

    #[test]
    fn test_preview_type_mutual_exclusivity() {
        let temp = TempDir::new().unwrap();

        // Create different file types
        let text_path = temp.path().join("file.txt");
        let image_path = temp.path().join("file.png");
        let binary_path = temp.path().join("file.exe");

        fs::write(&text_path, "text content").unwrap();
        create_test_png(&image_path);
        fs::write(&binary_path, [0x4D, 0x5A]).unwrap(); // MZ header

        // Text file: is_text=true, is_image=false, is_binary=false
        assert!(is_text_file(&text_path));
        assert!(!is_image_file(&text_path));
        assert!(!is_binary_file(&text_path));

        // Image file: is_text=false, is_image=true, is_binary=false
        assert!(!is_text_file(&image_path));
        assert!(is_image_file(&image_path));
        assert!(!is_binary_file(&image_path));

        // Binary file: is_text=false, is_image=false, is_binary=true
        assert!(!is_text_file(&binary_path));
        assert!(!is_image_file(&binary_path));
        assert!(is_binary_file(&binary_path));
    }

    #[test]
    fn test_directory_is_not_any_file_type() {
        let temp = TempDir::new().unwrap();
        let dir_path = temp.path().join("subdir");
        fs::create_dir(&dir_path).unwrap();

        // Directory should not match any file type
        assert!(!is_text_file(&dir_path));
        assert!(!is_image_file(&dir_path));
        // is_binary_file may return false or check differently for dirs
    }
}
