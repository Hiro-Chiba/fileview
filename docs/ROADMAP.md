...
- [ ] フォーカス切り替えロジック
  - `Tab`キーでトグル
  - プレビュー非表示時は常にTree
- [ ] PR: `feat: Add focus state management`
...
- [ ] その他のキー動作
  - `Enter`: Treeフォーカス時のみファイル操作
  - `Esc`: プレビューフォーカス → Treeに戻る
- [ ] PR: `feat: Implement focus-aware keyboard handling`
...
- [ ] 領域判定ロジック
  - レイアウト情報からクリック位置を判定
- [ ] PR: `feat: Implement mouse click focus switching`

### 14.4 視覚的フィードバック
**優先度:** 中

- [ ] フォーカスインジケーター
  - フォーカスのあるパネルのボーダーをハイライト
  - 色またはスタイル（太字など）で区別
- [ ] PR: `feat: Add focus visual indicator`
...
- [ ] フォーカス切り替えテスト
- [ ] キー操作のフォーカス対応テスト
- [ ] PR: `test: Add focus management tests`
